...
import { addClassworkAction, getEnrollmentsAction, getUsersAction } from '@/app/actions/dbActions';
import { toast } from 'sonner';
import { Loader2, Plus, Trash2, Calendar, Link as LinkIcon, X, UserCheck, Mail, ListChecks, FileText, CheckCircle2 } from 'lucide-react';
import { QuizQuestion, User } from '@/utils/storage';
...
export default function CreateClassworkDialog({ subjectId, onClose, onCreated }: CreateClassworkDialogProps) {
  ...
  const [submissionType, setSubmissionType] = useState<'file' | 'quiz'>('file');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addClassworkAction(payload);
      toast.success("Assessment deployed!");
      onCreated();
    } catch (err) {
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl rounded-[2.5rem] p-0 overflow-hidden border-none shadow-3xl flex flex-col h-[90vh] bg-white">
        ...
        <div className="px-8 py-4 flex-none flex items-center justify-between border-b border-slate-50">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-slate-900 uppercase tracking-tight">Create New Classwork</DialogTitle>
          </DialogHeader>
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-8 space-y-6 bg-slate-50/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground ml-1">Academic Title</Label>
                <Input 
                  value={title} 
                  onChange={e => setTitle(e.target.value)} 
                  required 
                  placeholder="e.g. Activity 1: Neural Links"
          ...
          <div className="space-y-1.5">
            ...
            <Textarea 
              ...
              placeholder="State the instructional objectives and requirements..." 
            />
          </div>
              ...
              <div className="space-y-6">
                {questions.length === 0 ? (
                   <div className="p-16 text-center border-4 border-dashed rounded-[2.5rem] border-slate-100 bg-white/50">
                      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Awaiting question initialization</p>
                   </div>
                ) : (
                  questions.map((q, idx) => (
            ...
            {submissionType === 'file' && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 ml-1">
                  ...
                  <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Supplementary Assets</Label>
                </div>
                <div className="space-y-3">
        ...
        <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-100 flex-none flex items-center justify-between px-8 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-4">
               <Checkbox id="notify-create" checked={notifyStudents} onCheckedChange={(v) => setNotifyStudents(!!v)} className="h-5 w-5 border-primary rounded-md" />
               <label htmlFor="notify-create" className="flex items-center gap-2 font-black uppercase text-[9px] tracking-widest text-slate-500 cursor-pointer hover:text-primary transition-colors">
            ...
            <div className="flex items-center gap-5">
               ...
               <Button onClick={handleSubmit} disabled={loading} className="h-11 px-10 rounded-xl bg-primary text-white font-black uppercase text-[10px] tracking-[0.2em] shadow-xl transition-all active:scale-95 gap-3">
                 {loading ? <Loader2 className="animate-spin h-4 w-4" /> : <Plus size={16} />} Deploy Assessment
               </Button>
            </div>
        </div>
      </DialogContent>
